package net.friendly_bets.providers;

public final class ExternalProviderIds {

    /** Canonical external provider ids (Team.externalAliases.provider / LayerProviderRegistry). */
    public static final String MARATHONBET = "marathonbet";
    public static final String MELBET = "melbet";
    public static final String TWENTYFOUR_SCORE = "24score.pro";
    public static final String CHAMPIONAT = "championat.com";
    public static final String EURO_FOOTBALL = "euro-football.ru";
    public static final String SOCCER365 = "soccer365.ru";
    public static final String SPORTS_RU = "sports.ru";
    public static final String FOOTBALL24 = "football24.ua";
    public static final String RUSCORE = "ruscore.ru";
    public static final String FLASHSCORE = "flashscorekz.com";
    public static final String FLASHSCORE_UA = "flashscore.com.ua";
    public static final String LIVERESULT = "liveresult.ru";

    private ExternalProviderIds() {
    }
}
