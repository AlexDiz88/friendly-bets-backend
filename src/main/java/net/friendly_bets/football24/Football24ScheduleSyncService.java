...
public class Football24ScheduleSyncService implements ScheduleProvider {

    private static final Logger log = LoggerFactory.getLogger(Football24ScheduleSyncService.class);

    @Override
    public String providerId() {
        return ExternalProviderIds.FOOTBALL24;
    }
    ...
    public ScheduleSyncResultDto syncByLeagueCode(String leagueCodeRaw, Integer matchday) {
        League.LeagueCode leagueCode = Football24TeamNamesService.parseLeagueCode(leagueCodeRaw);
    ...
    private ScheduleSyncResultDto syncLeagueBody(
        ...
        League.LeagueCode leagueCode = league.getLeagueCode();
        int leagueId = teamNamesService.requireLeagueId(leagueCode);
        ...
        if (explicitMatchday != null) {
            ...
            if (!formatSlots.isEmpty()) {
                ...
                if (nextSlot.isPresent()) {
                    ...
                    log.info("football24 sync {}: next matchday after {} missing — OK", leagueCode, currentOrder);
                }
            } else {
        ...
        if (respectFarKickoffSkip && farKickoffSkipSupport.shouldSkip(
            ...
            log.info("football24 sync {}: DB data present and earliest kickoff farther than {} days — skip without HTTP",
        ...
        Instant seasonsReqAt = Instant.now();
        long seasonsT0 = System.currentTimeMillis();
        String seasonsJson;
        try {
            seasonsJson = httpClient.fetchSeasonsJson(leagueId);
            httpLogs.add(ExternalApiMonitoringService.httpLog(
                    "SEASONS",
                    String.valueOf(leagueId),
                    200,
                    "SUCCESS",
                    System.currentTimeMillis() - seasonsT0,
                    null,
                    null,
                    seasonsReqAt
            ));
        ...
        int football24SeasonId = seasonIdOpt.getAsInt();
        String slug = properties.getTournamentSlugs().get(leagueCode.name());

        Instant fixturesReqAt = Instant.now();
        long fixturesT0 = System.currentTimeMillis();
        String fixturesJson;
        try {
            fixturesJson = httpClient.fetchFixturesRoundsJson(football24SeasonId, slug);
            httpLogs.add(ExternalApiMonitoringService.httpLog(
                    "FIXTURES_ROUNDS",
                    String.valueOf(football24SeasonId),
                    200,
                    "SUCCESS",
                    System.currentTimeMillis() - fixturesT0,
                    null,
                    null,
                    fixturesReqAt
            ));
        ...
        Football24ParsedSchedule parsed = scheduleParser.parseFixturesRounds(fixturesJson, football24SeasonId);
        ...
        for (Football24ParsedSchedule.Round round : parsed.getRounds()) {
            ...
            for (Football24ParsedSchedule.Match match : round.getMatches()) {
                Optional<Team> home = teamAliasResolver.resolveByProviderName(
                        ExternalProviderIds.FOOTBALL24, match.getHomeName());
                Optional<Team> away = teamAliasResolver.resolveByProviderName(
                        ExternalProviderIds.FOOTBALL24, match.getAwayName());
    ...
    private void upsertMatch(
            ...
            String homeTeamId,
            String awayTeamId,
            Football24ParsedSchedule.Match match,
            Instant fetchedAt
    ) {
