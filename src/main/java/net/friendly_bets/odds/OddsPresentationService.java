...
public class OddsPresentationService {
    ...
    public OddsEventMarketsDto getMarketsForMatchSchedule(String matchScheduleId) {
        ...
        Optional<Odds> oddsSnapshot = oddsService.findByMatchScheduleId(matchScheduleId);
        if (oddsSnapshot.isEmpty()
                || oddsSnapshot.get().getMarketGroups() == null
                || oddsSnapshot.get().getMarketGroups().isEmpty()) {
            throw new BadRequestException("oddsNotAvailable");
        }

        List<String> presentationBookmakers = resolvePresentationBookmakers(oddsSnapshot);

        List<OddsMarketGroup> presentationGroups = new ArrayList<>(oddsSnapshot.get().getMarketGroups());
        ...
        Instant fetchedAt = oddsSnapshot.get().getFetchedAt() != null
                ? oddsSnapshot.get().getFetchedAt()
                : now;
        return toDto(match, presentationGroups, fetchedAt, presentationBookmakers);
    }

    private List<String> resolvePresentationBookmakers(Optional<Odds> oddsSnapshot) {
        if (oddsSnapshot.isPresent()) {
            List<String> fromDoc = oddsSnapshot.get().getBookmakers();
            if (fromDoc != null && !fromDoc.isEmpty()) {
                return new ArrayList<>(fromDoc);
            }
        }
    ...
    private void prepareMarketGroupsForPresentation(List<OddsMarketGroup> groups, List<String> bookmakers) {
        ...
        oddsService.enrichBetTitles(groups);
